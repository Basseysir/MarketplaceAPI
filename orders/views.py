...
from .serializers import OrderSerializer, SellerOrderSerializer, SellerOrderStatusSerializer


# API View for handling the checkout process
class CheckoutView(APIView):
    """Create an order from the authenticated buyer's shopping cart."""
    permission_classes = [IsAuthenticated]
    
    @transaction.atomic
    def post(self, request):
        """Process the checkout flow and create order records."""
        # Get buyer info
        print(f"\nBuyer: {request.user.email}")
        
        # Retrieve cart and validate it's not empty
...
class SellerOrdersView(APIView):
    """Return seller-specific orders for the authenticated seller."""
    permission_classes = [IsAuthenticated]
    def get(self, request):
        if request.user.role != "seller":
            return Response({"error": "Only sellers can access"}, status=status.HTTP_403_FORBIDDEN)
        orders = SellerOrder.objects.filter(seller=request.user.seller_profile)
        serializer = SellerOrderSerializer(orders, many=True)
        return Response(serializer.data)

class MyOrdersView(APIView):
    """Return all orders created by the authenticated buyer."""
    permission_classes = [IsAuthenticated]
    def get(self, request):
...
class SellerOrderUpdateView(APIView):
    """Allow a seller to update the status of their own seller order."""
    permission_classes = [IsAuthenticated]
    def patch(self, request, pk):
        seller_order = SellerOrder.objects.get(id=pk, seller=request.user.seller_profile)
        ...
        return Response({"message": "Order updated successfully", "status": seller_order.status})