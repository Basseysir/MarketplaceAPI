...
class SellerDashboardView(APIView):
    ...
    def get(self, request):
        ...
        processing_orders = SellerOrder.objects.filter(seller=seller, status="processing").count()
        delivered_orders = SellerOrder.objects.filter(seller=seller, status="delivered").count()
        revenue = sum(order.subtotal for order in SellerOrder.objects.filter(seller=seller))
